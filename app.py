...
if uploaded_file:
    ...
    try:
        # ---------------------------------------------------------
        # 1. READ RAW SURVEY DATA (PURE SOURCE OF TRUTH)
        # ---------------------------------------------------------
        ...
        # Map dynamic column headers to clean SKU names
        ...
        try:
            ...
            for _, row in df_targets.iterrows():
                ...
                # Normalize Excel date formatting bugs dynamically (e.g. 7-11 parsed as dates)
        ...
        # ---------------------------------------------------------
        # 3. DYNAMICALLY BUILD CHANNEL SUMMARY
        # ---------------------------------------------------------
        # Extract all unique channels directly from the survey response data
        raw_channels = df['店鋪_clean'].unique().tolist()
        
        # Consolidate store channels dynamically
        consolidated_counts = {}
        ...
        # ---------------------------------------------------------
        # WEB DISPLAY: DASHBOARD & PLOTLY DONUT
        # ---------------------------------------------------------
        st.divider()
        ...
        # ---------------------------------------------------------
        # DYNAMIC EXCEL EXPORT WORKBOOK GENERATOR
        # ---------------------------------------------------------
        st.divider()
        ...
        with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
            ...
            # Helper function to generate dynamic SKU analysis for any filtered DataFrame
